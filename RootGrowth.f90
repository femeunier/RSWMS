...
!********************************************************************************
!> originates new axes
SUBROUTINE Newaxs(sumgrw,ngrwnw)
  USE typedef
  USE RootData
  USE ParamData
  ...
  REAL(dp) :: newmas,newlen,sLoc  !,MPL
  REAL(sp) ::  t,tloc,alpha,v,tgrow
  REAL(sp) ::  dx,dy,dz,dxini,dyini,dzini,dxstr,dystr,dzstr,kxx,kxy,kxz,kyx,kyy,kyz,kzx,kzy,kzz
  ...
  DO i = 1,nnewax(kaxemg)
	 ...
	 IF (l_conduc) THEN
	 CALL Ssgcom2(corner,1._dp,kxx,kxy,kxz,kyx,kyy,kyz,kzx,kzy,kzz)
	 ELSE 
     ...
     alpha = ran(seed)*2._sp*pi
     CALL Initial(tLoc,1,iaxis(ngrwnw),1._dp,alpha,inaxs,dxini,dyini,dzini)
  ...
  dz = dzini
     ...
     CALL Length(v,t-tnewax(kaxemg),newlen,corner)
...
SUBROUTINE Brdevl(t,sumgrw,igrow,ngrwnw)
  ...
  REAL(dp) :: newlen,newmas,sumgrw,sLoc   !,MPL
...
1    ivch = ivch-1
  ...
  IF (Tpot.EQ.0) THEN
      fac_stress=1.
  ELSE
      fac_stress=ABS(Tact(1)/Tpot)
  END IF
  
  IF (fac_stress.LT.0.) THEN
	fac_stress=0.
  ELSEIF (fac_stress.GT.1.) THEN
	fac_stress=1.
  END IF
...
SUBROUTINE CheckMacro(corner,lMacro)
  ...
  lMacro=.false.
  DO i=1,8
   IF (s(corner(i)).EQ.0) THEN
   print*,'corner(i)',i,corner(i)
      lMacro=.true.
  ENDIF
  END DO
  RETURN
...
SUBROUTINE Length(v,tgrow,newlen,corner)
  USE typedef
  USE TempData
  USE SolData, ONLY: theta,par,MatNum
  USE ConData
  USE StrData
  USE RootData, ONLY: ltemp,tnewax,ltoxi
  IMPLICIT NONE

  REAL(sp), INTENT(in) ::  v,tgrow
  REAL(sp) :: cncimp,temimp,strimp
  REAL(sp) :: theta1=0.1,theta2=0.3
  ...
  ! calculer theta-moyen
  thetaMean = sum(theta(corner(1:8)))/8
  print*,thetaMean

  ! interpolation linéaire
  IF (MatNum(corner(1)) == 1) THEN
     thr = par(2,1)
     ths = par(3,1)
 ElSE
    thr = par(2,2)
...
print*,ths
 ...
 IF (thetaMean < 4*thr) THEN
    impwc = thetaMean
    ELSE
      IF (thetaMean >= 4*thr .AND. thetaMean <= ths) THEN
      impwc = 1
      ELSE
      impwc = ths/thetaMean
      END IF
END IF
 ...
 print*,impwc
  ...
  ELSE
     cncimp = 1._sp
  ENDIF
  newlen = DBLE((cncimp*strimp*temimp*impwc)*v*tgrow)
...
SUBROUTINE Ssgcom2(corner,stndrd,kxx,kxy,kxz,kyx,kyy,kyz,kzx,kzy,kzz)
  USE typedef
  USE GridData
  USE StrData
  USE ParamData, ONLY: pi
  ...
  REAL(sp) :: Rsx1,Rsy1,Rsz1,Rsx2,Rsy2,Rsz2,phi,totstr 
  REAL(dp), INTENT(in) :: stndrd
  INTEGER(ap), INTENT(in) :: corner(8)
  ...
  !> conductivity1: othogonal
  !> calculate averaged & normalized soil resistivity per cube section (harmonic mean of the resistance in each corner point)
 
	Rsx1=(4._sp/smax)/((1/s(corner(1)))+(1/s(corner(3)))+&
	(1/s(corner(5)))+(1/s(corner(7))))
	Rsx2=(4._sp/smax)/((1/s(corner(2)))+(1/s(corner(4)))+&
	(1/s(corner(6)))+(1/s(corner(8))))
	Rsy1=(4._sp/smax)/((1/s(corner(1)))+(1/s(corner(2)))+&
	(1/s(corner(5)))+(1/s(corner(6))))
	Rsy2=(4._sp/smax)/((1/s(corner(3)))+(1/s(corner(4)))+&
	(1/s(corner(7)))+(1/s(corner(8))))
	Rsz1=(4._sp/smax)/((1/s(corner(1)))+(1/s(corner(2)))+&
	(1/s(corner(3)))+(1/s(corner(4))))
	Rsz2=(4._sp/smax)/((1/s(corner(5)))+(1/s(corner(6)))+&
	(1/s(corner(7)))+(1/s(corner(8))))
	
	kxx_lin=((2._sp)/(Rsx1+Rsx2))
	kyy_lin=((2._sp)/(Rsy1+Rsy2))
	kzz_lin=((2._sp)/(Rsz1+Rsz2))
	
	  !> normalize the 'soil conductivity' with the total strength

	totstr=(SQRT(kxx_lin**2+kyy_lin**2+kzz_lin**2))	
	
	IF ((ABS(kxx_lin).GT.1.E-10_sp).OR.(ABS(kyy_lin).GT.1.E-10_sp).OR.(ABS(kzz_lin).GT.1.E-10_sp)) THEN	
	kxx_lin= aint((kxx_lin/totstr) * 1.E4_sp)/1.E4_sp
	kyy_lin= aint((kyy_lin/totstr) * 1.E4_sp)/1.E4_sp
	kzz_lin= aint((kzz_lin/totstr) * 1.E4_sp)/1.E4_sp
	ENDIF
  ...
  !> conductivity2: x is orthogonal, y&z in 45-degree angle
  !> calculate averaged & normalized soil resistivity per cube section (harmonic mean of the resistance in each corner point)
	...
	Rsy1= (4._sp/smax)/(1/s(corner(3))+1/s(corner(4))+1/(2*s(corner(1)))+&
		1/(2*s(corner(2)))+1/(2*(s(corner(7)))+1/(2*s(corner(8)))))	
	Rsy2= (4._sp/smax)/(1/s(corner(5))+1/s(corner(6))+1/(2*s(corner(1)))+&
		1/(2*s(corner(2)))+1/(2*(s(corner(7)))+1/(2*s(corner(8)))))	

	Rsz1= (4._sp/smax)/(1/s(corner(1))+1/s(corner(2))+1/(2*s(corner(3)))+&
		1/(2*s(corner(4)))+1/(2*(s(corner(5)))+1/(2*s(corner(6)))))	
	Rsz2= (4._sp/smax)/(1/s(corner(7))+1/s(corner(8))+1/(2*s(corner(3)))+&
		1/(2*s(corner(4)))+1/(2*(s(corner(5)))+1/(2*s(corner(6)))))	
	

	
	!> calculate the conductivities along the diagonal principal axes
	kxx_skew_x=((2._sp)/(Rsx1+Rsx2))
	kyy_skew_x=((2._sp)/(Rsy1+Rsy2))
	kzz_skew_x=((2._sp)/(Rsz1+Rsz2))
	...
	kxx_skew_x= aint((kxx_skew_x/totstr)  * 1.E4_sp)/1.E4_sp
	kyy_skew_x= aint((kyy_skew_x/totstr)  * 1.E4_sp)/1.E4_sp
	kzz_skew_x= aint((kzz_skew_x/totstr)  * 1.E4_sp)/1.E4_sp
	ENDIF
  ...
  !> conductivity3: y is orthogonal, x&z in 45-degree angle
  !> calculate averaged & normalized soil resistivity per cube section (harmonic mean of the resistance in each corner point)
 	
	Rsx1= (4._sp/smax)/(1/s(corner(1))+1/s(corner(3))+1/(2*s(corner(4)))+&
		1/(2*s(corner(2)))+1/(2*(s(corner(5)))+1/(2*s(corner(7)))))	
	Rsx2= (4._sp/smax)/(1/s(corner(6))+1/s(corner(8))+1/(2*s(corner(4)))+&
		1/(2*s(corner(2)))+1/(2*(s(corner(5)))+1/(2*s(corner(7)))))		
	...
	Rsz1= (4._sp/smax)/(1/s(corner(2))+1/s(corner(4))+1/(2*s(corner(1)))+&
		1/(2*s(corner(3)))+1/(2*(s(corner(6)))+1/(2*s(corner(8)))))	
	Rsz2= (4._sp/smax)/(1/s(corner(5))+1/s(corner(7))+1/(2*s(corner(1)))+&
		1/(2*s(corner(3)))+1/(2*(s(corner(6)))+1/(2*s(corner(8)))))	

	
	!> calculate the conductivities along the diagonal principal axes
	kxx_skew_y=((2._sp)/(Rsx1+Rsx2))
	kyy_skew_y=((2._sp)/(Rsy1+Rsy2))
	kzz_skew_y=((2._sp)/(Rsz1+Rsz2))
	
	  !> normalize the 'soil conductivity' with the total strength (from the orthogonal conductivity tensor) 
	IF ((ABS(kxx_skew_y).GT.1.E-10_sp).OR.(ABS(kyy_skew_y).GT.1.E-10_sp).OR.(ABS(kzz_skew_y).GT.1.E-10_sp)) THEN	
	kxx_skew_y= aint((kxx_skew_y/totstr)  * 1.E4_sp)/1.E4_sp
	kyy_skew_y= aint((kyy_skew_y/totstr)  * 1.E4_sp)/1.E4_sp
	kzz_skew_y= aint((kzz_skew_y/totstr)  * 1.E4_sp)/1.E4_sp
	ENDIF
  ...
  !> conductivity4: z is orthogonal, x&y in 45-degree angle 
  !> calculate averaged & normalized soil resistivity per cube section (harmonic mean of the resistance in each corner point)
 
	Rsx1= (4._sp/smax)/(1/s(corner(1))+1/s(corner(5))+1/(2*s(corner(2)))+&
		1/(2*s(corner(6)))+1/(2*(s(corner(3)))+1/(2*s(corner(7)))))		
	Rsx2= (4._sp/smax)/(1/s(corner(4))+1/s(corner(8))+1/(2*s(corner(2)))+&
		1/(2*s(corner(6)))+1/(2*(s(corner(3)))+1/(2*s(corner(7)))))		
		
	Rsy1= (4._sp/smax)/(1/s(corner(2))+1/s(corner(6))+1/(2*s(corner(1)))+&
		1/(2*s(corner(5)))+1/(2*(s(corner(4)))+1/(2*s(corner(8)))))	
	Rsy2= (4._sp/smax)/(1/s(corner(3))+1/s(corner(7))+1/(2*s(corner(1)))+&
		1/(2*s(corner(5)))+1/(2*(s(corner(4)))+1/(2*s(corner(8)))))		
	...
	!> calculate the conductivities along the diagonal principal axes
	kxx_skew_z=((2._sp)/(Rsx1+Rsx2))
	kyy_skew_z=((2._sp)/(Rsy1+Rsy2))
	kzz_skew_z=((2._sp)/(Rsz1+Rsz2))
	
	  !> normalize the 'soil conductivity' with the total strength (from the orthogonal conductivity tensor) 
	IF ((ABS(kxx_skew_z).GT.1.E-10_sp).OR.(ABS(kyy_skew_z).GT.1.E-10_sp).OR.(ABS(kzz_skew_z).GT.1.E-10_sp)) THEN	
	kxx_skew_z= aint((kxx_skew_z/totstr)  * 1.E4_sp)/1.E4_sp
	kyy_skew_z= aint((kyy_skew_z/totstr)  * 1.E4_sp)/1.E4_sp
	kzz_skew_z= aint((kzz_skew_z/totstr)  * 1.E4_sp)/1.E4_sp
	ENDIF
	...
	ortho1 = MAXVAL(linmatrix)
	ortho2 = MAXVAL(linmatrix, mask = linmatrix.LT.ortho1)
	skew_x1 = MAXVAL(skewmatrix_in_x)
	skew_x2 = MAXVAL(skewmatrix_in_x, mask = skewmatrix_in_x.LT.skew_x1)
	skew_y1 = MAXVAL(skewmatrix_in_y)
	skew_y2 = MAXVAL(skewmatrix_in_y, mask = skewmatrix_in_y.LT.skew_y1)
	skew_z1 = MAXVAL(skewmatrix_in_z)
	skew_z2 = MAXVAL(skewmatrix_in_z, mask = skewmatrix_in_z.LT.skew_z1)
...
SUBROUTINE Initial(tLoc,iord,iax,stndrd,alpha,inaxs,dxini,dyini,dzini)
  USE typedef
  USE typedef
  USE GeoData
  USE RootData, ONLY: seed
...
SUBROUTINE Nwcomp(igrow,corner,newlen,dx,dy,dz,tempage)
  ...
  gamma = (2*ran(seed)-1)*2._sp*pi
  
 !> add a random angle with the expected value 0 and a calc stdev to the old heading angle
 !> calc stdev from a random sample from a normal(Gaussian) distribution scaled with the segment length and the range of rdmang
    stdev = (stndrd)**0.5*(rdmang(ordgrw(igrow)))
  ...
  !> get soil strength gradient components:
	 IF (l_conduc) THEN
	 CALL Ssgcom2(corner,stndrd,kxx,kxy,kxz,kyx,kyy,kyz,kzx,kzy,kzz)
	 ELSE 
 ...
 IF (l_conduc) THEN 
  ...
  dx = (kxx*drivex+kxy*drivey+kxz*drivez)
  dy = (kyx*drivex+kyy*drivey+kyz*drivez)
  dz = (kzx*drivex+kzy*drivey+kzz*drivez)
 ...
 END IF
  ...
  tgrow = dtroot
  CALL Length(v,tgrow,newlen,corner)

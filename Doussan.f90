 ...
 SUBROUTINE ConductRoot(t)
   ...
   DO irecn=1,nrec
	 ...
	 Lr(irecn)=0.
      ...
      !IF (irecn.EQ.577) print*,segage,Lr(irecn),t
 ...
 SUBROUTINE SSFdis(kout)
   ...
   IF (.NOT.lSUF.OR.kout.EQ.0) THEN
	 ...
	 print*,1
         ...
         ! solve with biconjugated gradient method (Numerical recipies)
         print*,2
         maxerr=min(1.e-6_dp,1.e-10_dp*nrec)
1717	 maxerr=maxerr/10.
   ...
   DO ipl=1,nplant 
      ...
      ENDIF
   ENDDO ! plant loop
   IF (isnan(Kcomp).OR.(Kcomp.EQ.(Kcomp-1))) Kcomp=Krs
   DO i=1,nElm
        IF (isnan(SSF(i)).OR.(SSF(i).EQ.(SSF(i)-1))) SSF(i)=0.
   ENDDO
   SSF=SSF/sum(SSF)
   !print*,sum(SSF),Krs,Kcomp
